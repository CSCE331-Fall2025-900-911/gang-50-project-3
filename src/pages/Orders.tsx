...
export default function Orders() {
  ...
  const addIngredient = (ingRaw: any) => {
    ...
    setCart(prev => prev.map((d: any) => {
      if (d.cart_id !== drinkId) return d;
      if (singleSelectCategories.includes(ing.ingredient_category_name)) {
        return { ...d, ingredients: { ...d.ingredients, [ing.ingredient_category_name]: ing } };
      }
  ...
  const removeIngredient = (drinkId: any, catName: string, ingId: any) => {
    setCart((prev: any[]) =>
      prev.map((d: any) => {
        ...
        return { ...d, extras: (d.extras as any[]).filter((e: any) => e.ingredient_ID !== ingId) };
      })
    );
  };
  ...
  const setCustomizationOption = (category: string, ing: any, drink: any) => {
    setCart(prev =>
      prev.map((d: any) => {
        if (d.cart_id !== drink.cart_id) return d;
        return { ...d, ingredients: { ...d.ingredients, [category]: ing } };
      })
    );
    if (customizingDrink?.cart_id === drink.cart_id) {
      setCustomizingDrink({
        ...customizingDrink,
  ...
  return (
    <div className="orders-layout">
              ...
              <div className="item-grid">
                {groupedIngredients[catName].map((item: any) => (
                  <button
                    ...
                    onClick={() => addIngredient(item)}
                    className={`item-card ${
                      cart.some((d: any) => d.extras.some((ex: any) => ex.ingredient_ID === item.ingredient_ID))
                        ? 'selected'
                        : ''
                    }`}
                  >
          ...
          cart.map((d: any) => (
            <div key={d.cart_id} className="order-drink">
              ...
              <div className="order-ingredients">
                ...
                {d.extras.map((ex: any) => (
                  <div key={ex.ingredient_ID} className="order-line">
                    <span>{ex.ingredient_name} (+${ex.ingredient_cost})</span>
                    <button onClick={() => removeIngredient(d.cart_id, 'extras', ex.ingredient_ID)}>×</button>
                  </div>
                ))}
              </div>
              <button className="btn" onClick={() => { setCustomizingDrink(d); setShowCustomizationPopup(true); }} style={{ marginTop: '0.5rem' }}>
                Customize
              </button>
            </div>
          ))
      ...
      {showCheckoutPopup && (
        <div className="checkout-popup" style={{
          ...
          <div className="checkout-content" style={{
            ...
            {cart.map((d: any) => (
              <div key={d.cart_id} style={{ borderBottom: '1px solid #ccc', paddingBottom: '0.5rem', marginBottom: '0.5rem' }}>
                ...
                <div style={{ paddingLeft: '1rem', marginTop: '0.25rem' }}>
                  ...
                  {d.extras.map((ex: any) => <div key={ex.ingredient_ID}>{ex.ingredient_name} (+${ex.ingredient_cost.toFixed(2)})</div>)}
                </div>
              </div>
            ))}
            <div style={{ borderTop: '2px solid #000', paddingTop: '0.5rem', marginTop: '0.5rem' }}>
            ...
            <div style={{ marginTop: '1rem', textAlign: 'center' }}>
              <button className="btn" style={{ marginRight: '1rem' }} onClick={() => {
                const outOfStock = cart.some((d: any) => !d.item.in_stock);
                if (outOfStock) alert("Some items are out of stock. Please remove them from your order.");
                else { alert("Thank you for your order!"); setCart([]); setShowCheckoutPopup(false); }
              }}>Confirm</button>
              <button className="btn" onClick={() => setShowCheckoutPopup(false)}>Cancel</button>
            </div>
          </div>
      ...
      {/* --- Sticky Customization Popup per Drink (with radio bubbles) --- */}
      {showCustomizationPopup && customizingDrink && (
        <div className="customization-popup" style={{
          position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh',
          backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000
          ...
          <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '8px', width: '500px', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            {singleSelectCategories.map(cat => (
              <div key={cat} style={{ marginBottom: '1rem' }}>
                <h4>{cat}</h4>
                {groupedIngredients[cat]?.map((ing: any) => (
                  <label key={ing.ingredient_ID} style={{ display: 'block', cursor: 'pointer', marginBottom: '0.25rem' }}>
                    <input
                      type="radio"
                      name={`${cat}-${customizingDrink.cart_id}`}
                      checked={customizingDrink.ingredients[cat]?.ingredient_ID === ing.ingredient_ID}
                      onChange={() => setCustomizationOption(cat, ing, customizingDrink)}
