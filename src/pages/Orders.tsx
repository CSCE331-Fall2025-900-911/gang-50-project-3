...
export default function Orders() {
  ...
  const [showCustomizationPopup, setShowCustomizationPopup] = useState(false);
  const [customizingDrink, setCustomizingDrink] = useState<any>(null);
  ...
  const openCustomizationPopup = (item: any) => {
    setCustomizingDrink({
      item,
      selections: {}
    });
  ...
  const saveCustomization = () => {
    setSelectedItems([...selectedItems, customizingDrink]);
    closeCustomizationPopup();
  };

  const handleIngredientSelect = (categoryName: string, ingredient: any) => {
    ...
    <>
      ...
      <div className="orders-container">
        ...
        <div className="items-grid">
          {filteredItems.map((item: any) => (
            <div
              key={item.item_ID}
              className="item-card"
              onClick={() => openCustomizationPopup(item)}
            >
            ...
            <div className="customization-popup">
              ...
              <div className="ingredient-options">
                {ingredients.map((ing: any) => (
                  <div key={ing.ingredient_ID} className="ingredient-option">
                    <button
                      className={`option-btn ${
                        customizingDrink.selections[ing.ingredient_category_name]?.ingredient_ID ===
                        ing.ingredient_ID
                          ? 'selected'
                          : ''
                      }`}
                      onClick={() =>
                        handleIngredientSelect(ing.ingredient_category_name, ing)
                      }
                    >
                      {ing.ingredient_name}
                    </button>
              ...
              <div className="popup-actions">
                <button className="cancel-btn" onClick={closeCustomizationPopup}>
                  Cancel
                </button>
        ...
        <div className="current-order">
          <h3>Current Order</h3>
          <ul>
            {selectedItems.map((order: any, index: number) => (
              <li key={index}>
                {order.item.item_name}
                {Object.entries(order.selections).length > 0 && (
                  <ul className="selection-list">
                    {Object.entries(order.selections).map(([cat, val]: any) => (
                      <li key={cat}>
                        {cat}: {val.ingredient_name}
                      </li>
                    ))}
                  </ul>
                )}
