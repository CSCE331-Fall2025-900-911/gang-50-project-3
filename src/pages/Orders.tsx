...
    const isSingleSelect = singleSelectCategories.includes(ing.ingredient_category_name);

    if (isSingleSelect) {
        // Block mandatory single-select options from being added via the main grid
        return alert(`Please use the Customization menu (which opens automatically) to select ${ing.ingredient_category_name} options.`);
    }

    // Check if the last drink has its required single-select options set
    const isDrinkComplete = singleSelectCategories.every(cat => 
        lastDrink.ingredients[cat] !== null
    );

    if (!isDrinkComplete) {
        // Block the addition of the extra ingredient if the mandatory options are missing
        return alert("Please CONFIRM the required customization options (Size, Sweetness, etc.) for the last drink before adding any extras.");
    }
...
      // This path handles adding extras, now that validation has passed
      return {
        ...d,
        extras: [...d.extras, ing],
      };
...
    // --- NEW VALIDATION: Check for missing required selections ---
    const requiredSelections = singleSelectCategories.filter(cat => {
        return customizingDrink.ingredients[cat] === null;
    });

    if (requiredSelections.length > 0) {
        const missingCategories = requiredSelections.join(', ');
        alert(`Please select options for the following required categories: ${missingCategories}`);
        return; 
    }
    // --- END VALIDATION ---
