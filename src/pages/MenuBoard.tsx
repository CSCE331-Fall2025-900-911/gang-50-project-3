  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const menuRes = await fetch('/api/items');
        const itemsJson = await menuRes.json();
      ...
      <div className="customization-bar">
        {Object.entries(ingredients).map(([cat, items]) => (
          <div key={cat} className="customization-group">
            ...
            <div className="options-row">
              {items.map((ing: any) => (
                <span key={ing.ingredient_ID} className="option-item">{ing.ingredient_name}</span>
              ))}
            </div>
          </div>
        ))}
    ...
    <ul className="menu-list">
      {items.map((item) => (
        <li key={item.item_ID} className="menu-item">
          {item.item_name} {item.seasonal_item && <span className="new-tag-inline">NEW</span>}
        </li>
      ))}
    </ul>
  </div>
